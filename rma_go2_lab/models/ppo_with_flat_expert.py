    ...
    def __init__(self, checkpoint_path: str, activation: str = "elu", device: str = "cpu") -> None:
        ...
        for index, layer_id in enumerate(layer_ids):
            ...
            if index != len(layer_ids) - 1:
                modules.append((f"act_{layer_id}", _activation_from_name(act_name)))
...
class PPOWithFlatExpert(PPO):
    def __init__(
        ...
        *args,
        ...
        flat_imitation_loss_coef: float = 0.0,
        flat_imitation_height_range_threshold: float = 0.06,
        flat_imitation_command_threshold: float = 0.1,
        **kwargs,
    ) -> None:
        super().__init__(policy, *args, **kwargs)
        self.flat_imitation_loss_coef = float(flat_imitation_loss_coef)
        self.flat_imitation_height_range_threshold = float(flat_imitation_height_range_threshold)
        ...
        if flat_expert_path and self.flat_imitation_loss_coef > 0.0:
    ...
    def _flat_imitation_mask(self, obs_batch) -> torch.Tensor | None:
        if self.flat_expert is None:
            return None
        if "privileged" not in obs_batch.keys() or "policy" not in obs_batch.keys():
            return None

        privileged = obs_batch["privileged"]
        policy_obs = obs_batch["policy"]

        terrain_scan = privileged[:, -187:]
        terrain_range = terrain_scan.max(dim=-1).values - terrain_scan.min(dim=-1).values
        flat_enough = terrain_range < self.flat_imitation_height_range_threshold

        command = policy_obs[:, 9:12]
        moving = torch.linalg.norm(command, dim=-1) > self.flat_imitation_command_threshold

        return (flat_enough & moving).float()
    ...
    def update(self) -> dict[str, float]:
        ...
        for (
            ...
            if self.flat_expert is not None and self.flat_imitation_loss_coef > 0.0:
                expert_obs_batch = obs_batch[:original_batch_size]
                mask = self._flat_imitation_mask(expert_obs_batch)
                if mask is not None:
                    ...
                    if torch.count_nonzero(mask) > 0:
                        ...
                        loss = loss + self.flat_imitation_loss_coef * imitation_loss
